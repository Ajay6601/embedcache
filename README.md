...
| Search-shaped (Zipf) query traffic on a real corpus | **~80%** hit rate across four real models ([VALIDATION.md](VALIDATION.md)) |
...
embedcache does one thing, language-agnostically, and provably correctly. Five tiers of evidence live in the repo, each regenerated by code under `experiments/` and (the first) re-run by CI on every push:
...
| [VALIDATION.md](VALIDATION.md) | correctness + RAG + agentic + search + budgets across **4 real models at 4 dims** | **this repo's Go source + live Wikipedia** | Ollama 384/768/1024-dim + Gemini 3072-dim |

The multi-model suite deliberately surfaced real backend behaviors and shows embedcache handling each correctly rather than hiding them: Gemini is not bitwise-deterministic (embedcache stabilizes repeats to one vector), Gemini rate-limits under burst, and small-context models reject oversized chunks - none are cache defects.

| Correctness claim | Evidence |
|---|---|
| Byte-exact responses, correct batch index mapping | 3,453 randomized-batch embeddings, 0 mismatches - covers the LiteLLM [#22659](https://github.com/BerriAI/litellm/issues/22659) wrong-vector failure class; re-verified on 4 real models |
...
# or Docker
docker build -t embedcache https://github.com/Ajay6601/embedcache.git
```
...
- **Multi-model validation** across four real backends at four dimensions.
...
3. **Request-log PII redaction** - opt-in hash-only logging for privacy-sensitive shops.
